...
const behaviorSchema = z.object({
  autoShowResults: z.boolean().default(false),
  autoShowResultsTime: z.number().nonnegative().default(0),
  onlyScrumMasterCanShowResults: z.boolean().default(false),
  lockVotes: z.boolean().default(true),
  notifyOnVote: z.boolean().default(true),
  notifyOnJoin: z.boolean().default(true),
  persistentLeave: z.boolean().default(false),
});
...
export default function BehaviorCard() {
  ...
  const { data, isLoading } = api.gameSettings.get.useQuery(undefined, { refetchOnWindowFocus: false });
  ...
  const form = useForm<BehaviorForm>({
    ...
    values: data?.settings
      ? {
          ...
          onlyScrumMasterCanShowResults: data.settings.onlyScrumMasterCanShowResults,
    ...
    <Form {...form}>
      ...
      >
          ...
          render={({ field }) => (
            <FormItem className="flex items-start justify-between rounded-md border p-4">
              <div className="space-y-0.5 pr-4">
                ...
                <FormDescription>Reveal after a timeout automatically.</FormDescription>
              </div>
              <FormControl>
          ...
          render={({ field }) => (
            <FormItem className="flex items-start justify-between rounded-md border p-4">
              <div className="space-y-0.5 pr-4">
                ...
                <FormDescription>Delay before automatic reveal.</FormDescription>
              </div>
              <FormControl>
                <Input
                  type="number"
          ...
          render={({ field }) => (
            <FormItem className="flex items-start justify-between rounded-md border p-4">
              <div className="space-y-0.5 pr-4">
                ...
                <FormDescription>Restrict reveal to Scrum Master.</FormDescription>
              </div>
              <FormControl>
                <Switch checked={field.value} onCheckedChange={field.onChange} />
              </FormControl>
          ...
          render={({ field }) => (
            <FormItem className="flex items-start justify-between rounded-md border p-4">
              <div className="space-y-0.5 pr-4">
                ...
                <FormDescription>Prevent changes once revealed.</FormDescription>
              </div>
              <FormControl>
                <Switch checked={field.value} onCheckedChange={field.onChange} />
              </FormControl>
          ...
          render={({ field }) => (
            <FormItem className="flex items-start justify-between rounded-md border p-4">
              <div className="space-y-0.5 pr-4">
                ...
                <FormDescription>Emit room notifications for votes.</FormDescription>
              </div>
              <FormControl>
                <Switch checked={field.value} onCheckedChange={field.onChange} />
              </FormControl>
          ...
          render={({ field }) => (
            <FormItem className="flex items-start justify-between rounded-md border p-4">
              <div className="space-y-0.5 pr-4">
                ...
                <FormDescription>Let the room know when a player joins.</FormDescription>
              </div>
              <FormControl>
                <Switch checked={field.value} onCheckedChange={field.onChange} />
              </FormControl>
          ...
          render={({ field }) => (
            <FormItem className="flex items-start justify-between rounded-md border p-4">
              <div className="space-y-0.5 pr-4">
                ...
                <FormDescription>Keep “left room” across sessions.</FormDescription>
              </div>
              <FormControl>
                <Switch checked={field.value} onCheckedChange={field.onChange} />
              </FormControl>
        ...
        <div className="md:col-span-2 flex justify-end">
          <Button type="submit" disabled={mutate.isPending || isLoading}>Save behavior</Button>
        </div>
      </form>
    </Form>
  );
