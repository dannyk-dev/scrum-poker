...
export const gameRouter = createTRPCRouter({
  ...
  invitePlayers: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const email of input.emails) {
        ...
        const roomName = ctx.db.room
          .findUnique({ where: { id: input.roomId } })
  ...
  acceptInvite: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const existing = await db.roomUser.findMany({
      ...
      const ru = await db.roomUser.create({
        data: {
  ...
  startGame: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!ru || ru.role !== "SCRUM_MASTER")
        throw new Error("Only Scrum Master can start the game");
